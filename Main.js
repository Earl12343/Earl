        ...
        if(document.getElementById('inputBookId').value){
            bookList.forEach(value => {
                if(document.getElementById('inputBookId').value == value.id){
                    ...
                    value.age          = document.getElementById('inputBookAge').value,
                    value.number        = document.getElementById('inputBookNumber').value,
                    value.religion        = document.getElementById('inputBookReligion').value,
                    value.isComplete    = 1
                }   
            });
            document.getElementById('id').value = ''
            ...
            var item = {
                ...
                age        : document.getElementById('inputBookAge').value,
                number      : document.getElementById('inputBookNumber').value,
                religion      : document.getElementById('inputBookReligion').value,
                isComplete  : 1,
            }
            bookList.push(item)
        }
    ...
    bookList.forEach(function (value, i){
        ...
        table.innerHTML += `
            <tr>
                ...
                <td><button class="btn btn-sm btn-warning" onclick="read(${value.id},'${value.title}','${value.author}','${value.year}','${value.number}','${value.age}',${value.religion})">
    ...
    bookList2.forEach(function (value2, i){
        ...
        table2.innerHTML += `
            <tr>
                ...
                <td>${value2.age}</td>
                <td>${value2.number}</td>
                <td>${value2.religion}</td>
                <td><button class="btn btn-sm btn-warning" onclick="read2(${value2.id},'${value2.title}','${value2.author}','${value2.year}','${value2.number}','${value2.age}'
                ,${value2.religion})">
                <i class="fa fa-check"></i>
                </button></td>
                <td>
...
function find(id){
    ...
    bookList.forEach(function (value){
        if(value.id == id){
            ...
            document.getElementById('inputBookAge').value = value.age
            document.getElementById('inputBookNumber').value = value.number
            document.getElementById('inputBookReligion').value = value.religion
        }
    })
}

function read(id1,title1,author1,year1,number1,age1,religion1){
    if(id1){
        var item = [{
            id          : id1,
            title       : title1,
...
function read2(id1,title1,author1,year1,number1,age1,religion1){
    if(id1){
        var item = [{
            id          : id1,
            title       : title1,
